...
namespace BookStore.Sys.Forms.Book
{
    public partial class frmCategory : Form
    {
        ...
        private void dgv_Load()
        {
            ...
            sys.Ds.Reset();
            //Load data
            sys.Db.LoadDataIntoDgv(dgv, sys.Ds, "BookCategory");
            //Thêm primary key
            sys.Ds.Tables["BookCategory"].PrimaryKey = new DataColumn[] { sys.Ds.Tables["BookCategory"].Columns[0] };
        }
        ...
        private void btnDelete_Click(object sender, EventArgs e)
        {
            if (MessageBox.Show("Bạn có muốn xóa?", "Hệ thống", MessageBoxButtons.YesNo, MessageBoxIcon.Question, MessageBoxDefaultButton.Button2) == DialogResult.Yes)
            {
                ...
                {
                    if (rowS["BookCategoryId"].ToString().Equals(txtID.Text.Trim()))
                    {
                ...
                {
                    MessageBox.Show("Vui lòng chọn mã danh mục để xóa!", "Hệ thống", MessageBoxButtons.OK, MessageBoxIcon.Error);
                    return;
                }
                DataRow row = sys.Ds.Tables["BookCategory"].Rows.Find(txtID.Text.Trim());
                if (row != null)
                {
                    row.Delete();
                    MessageBox.Show("Xóa thành công!", "Hệ thống", MessageBoxButtons.OK, MessageBoxIcon.Information);
                }
                sys.Db.Update(sys.Ds, "BookCategory");
            }
        }
        ...
        private void btnSave_Click(object sender, EventArgs e)
        {
            ...
            if (isAdd)
            {
                DataRow newRow = sys.Ds.Tables["BookCategory"].NewRow();
                if (newRow != null)
                {
                    ...
                    sys.Ds.Tables["BookCategory"].Rows.Add(newRow);
                    Update("Thêm", "BookCategory");
